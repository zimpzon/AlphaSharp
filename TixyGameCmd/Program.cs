...
namespace TixyGameCmd
{
    ...
    {
        static IGame game = new Tixy(5, 5);
        static ISkynet skynet = new TixySkynet(game);

        static void Run(int id)
        {
            var coach = new Coach();
            coach.Run(game, skynet, "c:\\temp\\zerosharp", new Args { TrainingSimulationCount = 200, TrainingSimulationMaxMoves = 70, TrainingEpisodeMaxMoves = 50 });
